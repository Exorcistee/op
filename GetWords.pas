...
IMPLEMENTATION
  ...
  PROCEDURE ToLowerCase(VAR Ch: CHAR); {Преобразование к нижнему регистру латинских и
    ...
    BEGIN  
      ...
      IF (Ch = 'Ь') OR (Ch = 'ь') THEN Ch := 'ь' ELSE
  ...
  PROCEDURE GetWord(VAR Word: STRING; VAR InFile: TEXT); {Получение слов, читая файл построчно, и выделяя символы до пробела}
  VAR
    Ch: CHAR;
    Space: BOOLEAN;
  BEGIN
    Word := '';
    Space := FALSE;
    WHILE NOT(Space) AND NOT(EOLN(InFile))
    DO
     BEGIN
        READ(InFile, Ch);
        ToLowerCase(Ch);
